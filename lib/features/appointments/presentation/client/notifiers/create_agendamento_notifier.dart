...
class CreateAgendamentoNotifier extends ChangeNotifier {
  ...
  Future<void> loadProgramacoes(int estabelecimentoId) async {
    ...
    try {
      final programacoes = await _repository.getProgramacoesByEstabelecimento(
        estabelecimentoId,
      );
      ...
      _programacoesState = ProgramacoesError(
  ...
  bool isDateAvailable(DateTime date) {
    ...
    final datasDisponiveis =
        (_programacoesState as ProgramacoesLoaded).datasDisponiveis;
  ...
  void addServico(int servicoId) {
    if (!_selectedServicosIds.contains(servicoId)) {
      ...
      _log.t(
        'Serviço adicionado: $servicoId (total: ${_selectedServicosIds.length})',
      );
      notifyListeners();
    }
  }
  ...
  void removeServico(int servicoId) {
    ...
    _log.t(
      'Serviço removido: $servicoId (total: ${_selectedServicosIds.length})',
    );
    notifyListeners();
  }
  ...
  Future<void> loadSlots(int estabelecimentoId, DateTime data) async {
    ...
    try {
      // Busca slots concatenados de todas as programações da data
      final slots = await _repository.getSlotsByData(estabelecimentoId, data);
      _log.d('${slots.length} slots encontrados');
      _slotsState = SlotsLoaded(slots);
    } catch (e) {
  ...
  Future<void> createAgendamento() async {
    ...
    _log.i(
      'Criando agendamento - Carro: $_selectedCarroId, Slot: ${_selectedSlot!.id}, Serviços: $_selectedServicosIds',
    );
    ...
    try {
      ...
      _createState = CreateAgendamentoError(
        e.toString().replaceAll('Exception: ', ''),
      );
    }
