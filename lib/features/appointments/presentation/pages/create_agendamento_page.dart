...
class _CreateAgendamentoPageState extends State<CreateAgendamentoPage> {
        ...
        builder: (context, notifier, child) {
          ...
          return Stepper(
            ...
            controlsBuilder: (context, details) {
              ...
              final isLoading = notifier.createState is CreateAgendamentoLoading;
            ...
            steps: [
              Step(
                ...
                subtitle: notifier.selectedServicosIds.isEmpty
                    ? null
                    : Text('${notifier.selectedServicosIds.length} selecionado(s)'),
              ...
              Step(
                ...
                subtitle: notifier.selectedDate == null
                    ? null
                    : Text(DateFormat('dd/MM/yyyy').format(notifier.selectedDate!)),
      ...
      ProgramacoesError(message: final msg) => Center(
          child: Column(
            children: [
              ...
              Text(msg),
              TextButton(
                onPressed: () => notifier.loadProgramacoes(widget.estabelecimentoId),
                child: const Text('Tentar novamente'),
              ),
            ],
          ),
        ),
      ProgramacoesLoaded() => _buildCalendarWithAvailableDates(notifier, colorScheme),
    };
  }
  ...
  Widget _buildCalendarWithAvailableDates(
    ...
    if (notifier.selectedDate != null && notifier.isDateAvailable(notifier.selectedDate!)) {
      initialDate = notifier.selectedDate!;
    } else if (firstAvailableDate.isAfter(today) || _isSameDay(firstAvailableDate, today)) {
  ...
  Widget _buildHorarioStep(
    ...
    return switch (state) {
      ...
      SlotsLoaded(programacao: final programacao) => programacao == null
          ? _buildNoSlots(colorScheme)
          : _buildSlotsList(programacao, notifier, colorScheme),
    };
  }
  ...
  Widget _buildSlotsList(
    ProgramacaoDiariaModel programacao,
    CreateAgendamentoNotifier notifier,
    ColorScheme colorScheme,
  ) {
    final slotsDisponiveis = (programacao.slots ?? []).where((s) => s.isDisponivel).toList();
  ...
  Widget _buildConfirmacaoStep(
    ...
    if (vehiclesNotifier.state is VehiclesLoaded && notifier.selectedCarroId != null) {
      ...
      children: [
        ...
        if (vehicleSelecionado != null)
          _buildConfirmationItem(
            ...
            title: 'Veículo',
            value: '${vehicleSelecionado.marca} ${vehicleSelecionado.modelo} - ${vehicleSelecionado.placa}',
            colorScheme: colorScheme,
          ),
        if (notifier.selectedDate != null)
          _buildConfirmationItem(
